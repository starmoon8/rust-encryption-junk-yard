...
function renderPost(int $id, string $title, string $message, ?string $mediaPath, string $name, string $created_at, bool $showReplyButton = true, bool $sticky = false, bool $locked = false, bool $deleted = false): string {
    ...
    $resLink = $showReplyButton ? '?post_id=' . $id : '#';
    $qLink = $showReplyButton ? '?post_id=' . $id . '#q' . $id : '#q' . $id;
    $hashLink = $showReplyButton ? '?post_id=' . $id . '#' . $id : '#' . $id;
    $replyLinkHtml = $showReplyButton ? '<a href="?post_id=' . $id . '">[Reply]</a>' : '';
    $icons = '';
    ...
    if ($showReplyButton && mb_strlen($message, 'UTF-8') > MAX_PREVIEW_CHARS) {
        ...
        $toolongHtml = '<span class="toolong">Post too long. Click <a href="?post_id=' . $id . '#' . $id . '">here</a> to view the full text.</span>';
    }
    $omittedHtml = '';
...
function renderReply(int $reply_id, string $message, string $name, string $created_at, int $post_id, bool $deleted = false, bool $truncate = false): string {
    ...
    if ($truncate && mb_strlen($message, 'UTF-8') > MAX_REPLY_PREVIEW_CHARS) {
        ...
        $toolongHtml = '<span class="toolong">Post too long. Click <a href="?post_id=' . $post_id . '#reply_' . $reply_id . '">here</a> to view the full text.</span>';
    }
    return '<div class="post reply" id="reply_' . $reply_id . '"><p class="intro"><a id="' . $reply_id . '" class="post_anchor"></a><span class="name">' . htmlspecialchars($name) . '</span> <time datetime="' . $timeInfo['datetime'] . '">' . $timeInfo['formatted'] . '</time>&nbsp;<a class="post_no" id="post_no_' . $reply_id . '" onclick="highlightReply(' . $reply_id . ')" href="?post_id=' . $post_id . '#' . $reply_id . '">No.</a><a class="post_no" onclick="citeReply(' . $reply_id . ')" href="?post_id=' . $post_id . '#q' . $reply_id . '">' . $reply_id . '</a></p><div class="files "></div><div class="body">' . $displayMessage . $toolongHtml . '</div></div><br/>';
}
...
$script_vars .= '; </script>';
$head_links = '<link rel="stylesheet" media="screen" href="stylesheets/style.css?v=0" title="Yotsuba B">
<link rel="alternate stylesheet" title="Yotsuba" href="stylesheets/yotsuba.css?v=0">
<link rel="alternate stylesheet" title="Dark" href="stylesheets/dark.css?v=0">
<link rel="alternate stylesheet" title="Miku" href="stylesheets/miku.css?v=0">
...
if ($post_id) {
    ...
    if ($post) {
        ...
        $og_url = 'https://' . $_SERVER['HTTP_HOST'] . '/?post_id=' . $post_id;
    }
}
...
$head_title = '<title>' . $meta_title . '</title>';
?>
<!doctype html>
<html>
<head>
...
<body>
    ...
    <?php if ($post_id): ?>
        <a name="top"></a>
        <div class="banner">Posting mode: Reply <a class="unimportant" href="./">[Return]</a> <a class="unimportant" href="#bottom">[Go to bottom]</a></div>
    <?php endif; ?>
    ...
    <?php if ($post_id !== null && $post_id > 0): ?>
        ...
        ?>
        <?php if (!$is_locked): ?>
        <form name="post" onsubmit="return doPost(this);" enctype="multipart/form-data" action="" method="post">
        ...
        <form name="postcontrols" action="" method="post">
            ...
            ?>
            <div id="thread-interactions"><span id="thread-links"><a id="thread-return" href="./">[Return]</a><a id="thread-top" href="#top">[Go to top]</a> </span><span id="thread-quick-reply"><a id="link-quick-reply" href="#">[Post a Reply]</a></span><div class="clearfix"></div>
        </form>
        <a name="bottom"></a>
    <?php else: ?>
        <form name="post" onsubmit="return doPost(this);" enctype="multipart/form-data" action="" method="post">
        ...
        <div class="pages">
            <?php
            $prevText = $page > 1 ? '<a href="?page=' . ($page - 1) . '">Previous</a>' : 'Previous';
            $nextText = $page < $totalPages ? '<a href="?page=' . ($page + 1) . '">Next</a>' : 'Next';
            echo $prevText . ' [';
